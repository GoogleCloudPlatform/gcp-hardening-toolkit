...
1. **Organization Policies:** Enforces 14 security controls across Security (CC), Availability (A), and Confidentiality (C) criteria through GCP organization policies.
...
## Incremental Compliance Pattern

This module implements an **Incremental Compliance** design that allows multiple compliance frameworks (SOC2, HIPAA, PCI-DSS, etc.) to coexist without conflicts.

**Key Features:**
- ✅ **Non-Destructive**: Detects existing policies and skips creation
- ✅ **Additive**: Only creates policies that don't already exist
- ✅ **Idempotent**: Safe to apply multiple times
- ✅ **Compatible**: Designed to work alongside HIPAA, PCI, CIS, and other compliance modules

**Quick Start with Existing Policies:**
```bash
# 1. Detect existing organization policies
./scripts/detect_existing_policies.sh

# 2. Merge generated file with terraform.tfvars
cat terraform.tfvars.existing_policies >> terraform.tfvars

# 3. Apply SOC2 compliance (skips existing policies)
terraform apply
```

**How It Works:**
1. Script queries GCP for existing organization policies
2. Generates `existing_policies` map in terraform.tfvars
3. Terraform skips creating policies that already exist
4. Only missing policies are created

**Learn More:** See [docs/INCREMENTAL_COMPLIANCE.md](docs/INCREMENTAL_COMPLIANCE.md) for detailed usage scenarios, policy compatibility matrix, and troubleshooting.

## What gets deployed

### Organization Policies (14 total)

**Security Controls (8 policies):**
...
* `sql.restrictAuthorizedNetworks` - Restricts Cloud SQL network access

**Availability Controls (1 policy):**
