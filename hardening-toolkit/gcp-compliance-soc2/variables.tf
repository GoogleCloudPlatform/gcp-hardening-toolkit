...
variable "allowed_domains" {
  description = "List of allowed domains for IAM policies"
  type        = list(string)
  default     = null
}
...
variable "existing_policies" {
  description = <<-EOT
    Map of existing organization policies to prevent duplicate creation.
    Key: constraint name (e.g., "constraints/iam.disableServiceAccountKeyCreation")
    Value: policy details (can be any object, only existence is checked)
    
    This is automatically populated by running: ./scripts/detect_existing_policies.sh
  EOT
  type        = map(any)
  default     = {}
}
